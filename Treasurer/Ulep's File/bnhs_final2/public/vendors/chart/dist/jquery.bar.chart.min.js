(function(a)
	{
		a.fn.animatedBarChart=function(c){
			var l=this;
			var q=a(l).attr("id");
			var h=a.extend(true,{},{data:[],params:{group_name:"group_name",name:"name",value:"value"},
				horizontal_bars:false,chart_height:400,colors:null,show_legend:true,legend:{position:LegendPosition.bottom,width:200},
				x_grid_lines:true,y_grid_lines:true,tweenDuration:300,bars:{padding:0.075,opacity:0.7,opacity_hover:0.45,
				disable_hover:false,hover_name_text:"name",hover_value_text:"value",},number_format:{format:",.2f",decimal:".",
				thousands:",",grouping:[3],currency:["$"]},margin:{top:0,right:35,bottom:20,left:70},rotate_x_axis_labels:
				{process:true,minimun_resolution:720,bottom_margin:15,rotating_angle:90,x_position:9,y_position:-3}},c);

			var r,m,d,u,f,j,g,t,k=0,n=false,o,b=[],s,p;function e(){return d3.axisBottom(f)}
			function i(){return d3.axisLeft(j)}l.init=function(){if(!h.colors){h.colors=[];
				var v=d3.scaleOrdinal(d3.schemeCategory10);for(var x=0;x<10;x++){h.colors.push(v(x))}}
				if(!h.bars.disable_hover){k=20}a.each(h.data,function(z,A){b.push(A[h.params.group_name])});b=a.unique(b);
				if(a(document).width()<h.rotate_x_axis_labels.minimun_resolution&&h.rotate_x_axis_labels.process)
					{h.margin.bottom=h.margin.bottom+h.rotate_x_axis_labels.bottom_margin;n=true}
				d3.formatDefaultLocale(h.number_format);p=d3.format(h.number_format.format);
			var w=a("#"+q);d=w.width();u=h.chart_height;var y=d3.select("#"+q);
