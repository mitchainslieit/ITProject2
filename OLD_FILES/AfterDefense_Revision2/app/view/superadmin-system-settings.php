...
if(isset($_POST['transfer-button-no'])){
	extract($_POST);
	$obj->transferButtonNo();
}
if(isset($_POST['transfer-button-yes'])){
	extract($_POST);
	$obj->transferButtonYes();
}
...
if(isset($_POST['date-grading-period'])){
	extract($_POST);
	$obj->dateGradingPeriod($g_end_date);
}
							...
							<div class="accordion-content"> 
								...
								if($queryCount->rowCount() > 0){
									...
									<div class="systemContainer">
										<form action="superadmin-system-settings" method="POST" class="validateChangesInForm" id="formGrading"></form>
										<div class="systemBox1">
											<div class="innerHeader">
												<span style="font-weight: 500 !important;">Set Grading Period:</span>
											</div>
											<div class="setGradingContainer">';
												
												$queryGrading=$this->conn->prepare("SELECT active_grading, g_end_date from system_settings WHERE sy_status='Current' LIMIT 1");
												$queryGrading->execute();
												$rowQueryGrading=$queryGrading->fetch(PDO::FETCH_ASSOC);
												$dateGrading = date("Y-m-d\TH:i:s", strtotime($rowQueryGrading['g_end_date']));
												$currGrading=$rowQueryGrading['active_grading'];
												$grading=['1st', '2nd', '3rd', '4th', 'Disabled'];
												if($queryGrading->rowCount() > 0){
													echo'
													<select name="active_grading" form="formGrading">
													';
													...
													echo '
													<select name="active_grading" id="" form="formGrading">
										...
										echo'
												<div name="content">
													<button name="opener" class="customButton"><i class="far fa-calendar-alt"></i></button>
													<div name="dialog" title="Set Date for Grading Period">
														<form action="superadmin-system-settings" method="POST" class="validateChangesInForm">
															<span>End Date: </span>
															<input type="datetime-local" name="g_end_date" value="'.$dateGrading.'">
															<button name="date-grading-period" class="customButton">Save <i class="fas fa-save fnt"></i></button>
														</form>
													</div>
												</div>
											</div>
										...
										<p class="tright">
											<button name="select-grading-period" class="customButton" form="formGrading">Save <i class="fas fa-save fnt"></i></button>
										</p>
									</div>
									<div class="systemContainer">
										<div class="systemBox1">
											<div class="innerHeader">
												<span style="font-weight: 500 !important;">Enable Transferring of Students:</span>
											</div>';
											$queryCheckTransfer=$this->conn->prepare("SELECT * from system_settings WHERE sy_status='Current' and student_transfer='Yes' LIMIT 1");
											$queryCheckTransfer->execute();
											$rowQueryCheckTransfer=$queryCheckTransfer->fetch(PDO::FETCH_ASSOC);
											if($queryCheckTransfer ->rowCount() > 0){
												echo'
												<form action="superadmin-system-settings" method="POST">
													<p class="tleft">
														<button name="transfer-button-no" class="customButtonYes"></button>
													</p>
												</form>';
											}else{
												echo'
												<form action="superadmin-system-settings" method="POST">
													<p class="tleft">
														<button name="transfer-button-yes" class="customButtonNo"></button>
													</p>
												</form>';
											}
											echo'
