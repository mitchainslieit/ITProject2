...
class studentFunct {
    ...
    public function updateProfile($prof_pic){       
        ...
        if($type=='image/jpg' || $type=='image/jpeg' || $type=='image/png' || $type=='image/gif') { 
            if(!file_exists($pathWithFile)){
                if($size < 5000000){
                    ...
                    if($sql3->execute(array(
                        ...
                        $this->alert("Success!", "You have successfully changed your profile picture", "success", "student-information");
                    }else{
                        $this->alert("Error!", "You have successfully changed your profile picture", "error", "student-information");
                    }
                }else{
                    $this->alert("Error!", "Your file is too large! Please Upload 5MB Size", "error", "student-information");
                }
            }else{  
            ...
            $this->alert("Error!", "Upload JPG , JPEG , PNG & GIF File Formate.....CHECK FILE EXTENSION", "error", "student-information");
        }
    }
        ...
        if ( password_verify($oldPassword, $user['password']) ) {
            if ($password === $repassword) {
                if (strlen($password) >= 8) {
                    ...
                    $this->alert("Success!", "You have successfully changed your password", "success", "student-information");
                } else {
                    $this->alert('Error!',"Invalid password. Password length must be 8 characters and above",'error','student-information');
                } 
            } else {
                $this->alert('Error!',"Password doesn't match",'error','student-information');
            }
        }else {
            $this->alert('Error!',"Old Password doesn't match",'error','student-information');
        }
    }
...
public function getChildGrade($id){
    ...
    $query = $this->conn->prepare("SELECT 
        title,
        post,
        CONCAT(adm_fname, ' ', adm_lname) 'name',
        attachment
        FROM
        announcements ann
        JOIN
        admin ON ann.post_adminid = admin.admin_id
        WHERE
        view_lim = '0' and holiday ='No' and post != 'NULL'
        UNION SELECT 
        title,
        post,
        CONCAT(fac_fname, ' ', fac_lname) 'name',
        ...
        view_lim LIKE '%3%' AND gr_sec = ?
        AND accc_id = ?");
    $query->bindParam(1, $secc_id[0]);
    $query->bindParam(2, $_SESSION['accid']);
    $query->execute(); 
    $result = $query->fetchAll();
    $count = $query->rowCount();
      if($count > 0) {
            ...
            $html .= '<td class="tleft custPad2 longText">';
                $html .= '<h3 class="att_title" style="text-align: left;">'.$row['post'].' - '.$row['name'].'</h3>';
