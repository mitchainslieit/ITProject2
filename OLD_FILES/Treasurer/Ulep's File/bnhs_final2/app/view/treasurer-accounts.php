							...
							<tbody>
								...
								foreach($run->getPaymentInfo() as $row){
									...
									<tr>
										...
										<td>
											<div name="content">
												<button class="customButton" name="opener" onclick="">Update</button>
												<div name="dialog" title="Add Payment">
													<form action="treasurer-accounts" method="POST" autocomplete="off" id="update-form">
														<label>
									...
									foreach ($fixed_range as $fix) {
										foreach($result_range as $row){
											...
											if ($fix['budget_name'] == $budget_name) {
												...
												$budg_namef = strtolower($budg_name);
												$input = '<input type="text" name="'.$budg_namef.'" placeholder="Enter Fees..."
												data-validation="number" data-validation-allowing="range[1;'.$fix['total_amount'].'],float" />';
												/*echo ' 
														<div class="innerCont">
