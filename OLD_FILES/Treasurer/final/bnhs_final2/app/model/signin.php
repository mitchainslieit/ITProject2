	...
	private function getName($acc_id, $acc_type) { 
		switch ($acc_type) {
			case 'Admin': 
				...
				$query->execute();
				$user = $query->fetch();
				return $user['adm_fname'].' '.$user['adm_midname'].'. '.$user['adm_lname'];
			break;
			case 'Faculty': 
				...
				$query->execute();
				$user = $query->fetch();
				return $user['fac_fname'].' '.$user['fac_midname'].'. '.$user['fac_lname'];
				break;
			case 'Student': 
				...
				break;
			case 'Parent': 
				$query = $this->conn->prepare("SELECT * from accounts JOIN guardian ON acc_idx = acc_id WHERE acc_id = ?");
				$query->bindParam(1, $acc_id);
				$query->execute();
				$user = $query->fetch();
				return $user['guar_fname'].' '.$user['guar_midname'].'. '.$user['guar_lname'];
				break;
			case 'Treasurer': 
				$query = $this->conn->prepare("SELECT * from accounts JOIN guardian ON acc_idx = acc_id WHERE acc_id = ?");
				$query->bindParam(1, $acc_id);
