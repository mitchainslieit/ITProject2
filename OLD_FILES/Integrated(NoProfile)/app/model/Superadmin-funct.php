...
class SAdminFunct{
	...
	private function getRankFunction($data) {
		$count = 0;
		$unique = $data; rsort($unique);
	...
	private function insert_rank() {
		$getSec = $this->conn->query("SELECT * FROM section WHERE sec_id = '1'");
		...
		foreach($resSec as $sec) {
			...
			foreach($getAvgAll as $data) { 
				...
				$grade[] = (int) $data['average'];
			}
			$info = $this->array_sort($info, 'average', SORT_DESC);
			$ranker = $this->getRankFunction($grade);
			$final_array = array();
