...
if ($('body').is('[class*="superadmin-"]')) {
	var sum_table = $('#sum-students').DataTable();
	$('.superadmin-sum-page form input.grade').on('input', function() {
		var remarks = $(this).parent().siblings('td').find('.remarks');
	...
	$('.superadmin-sum-page .student-summer button[name=submit-grades]').on('click', function(e) {
		e.preventDefault();
		swal({
			title: "Are you sure?",
			text: "You can no longer edit the grade(s) after you submit.",
			icon: "warning",
			buttons: true,
			dangerMode: true,
		}) .then((willSubmit) => {
			if (willSubmit) {
				$('.superadmin-sum-page .student-summer').submit();
			} else {
				swal("The form has not been submitted");
			}
		});
	});
