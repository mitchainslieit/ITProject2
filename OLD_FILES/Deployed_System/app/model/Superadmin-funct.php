...
class SAdminFunct{
	...
	private function promote_students(){
		...
		foreach($getPromotedStudents->fetchAll() as $pstud) {
			...
			if($pstud['remarks'] === 'Promoted') {
				...
				$update4 = "UPDATE student SET stud_status ='Not Enrolled', sec_stat = NULL, secc_id = NULL, year_level = '".$pstud['promote_yr_level']."' where stud_id ='".$pstud['stud_id']."'";
	...
	public function deleteFeeType($id, $table){
		try{
			...
			if($queryStat == 'Permanent'){
				...
				$sql3=$this->conn->prepare("DELETE FROM budget_info_temp WHERE bd_name=:bd_name");
				$sql3->execute(array(
					':bd_name'=>$budget_name
				));
	...
	public function advAcceptRequest(){
		...
		if($checkbox !== null){ 
			for($i=0;$i<count($checkbox);$i++){
				...
				if($req_dept === $prevDept || $prevDept == NULL){
	...
	public function showClasses(){
		$sql=$this->conn->prepare("SELECT *,fac_no, CONCAT(fac_fname,' ',fac_midname,' ',fac_lname) AS fullname, sec_name as 's_name', grade_lvl as 'gr_lvl', fac_id, sec_id as 'sectionid' FROM accounts join faculty ON acc_idz=acc_id JOIN section ON fac_id=fac_idv WHERE fac_adviser='Yes'");
		$sql->execute();
		if($sql->rowCount()>0){
			while($r=$sql->fetch(PDO::FETCH_ASSOC)){
				$data[]=$r;
	...
	public function addClass($sec_id, $fac_idv){	
		try {
			...
			if($prevDept === $fac_deptLatest || $prevDept == NULL){
				if($queryCheckDeactive->rowCount() > 0){
					...
					if($queryStat == 'Permanent' || $queryStat == NULL){
		...
		try {
			...
			if($queryStat == 'Permanent'){
				...
				if($sql->execute(array(
					...
					$queryAdviserUpdate=$this->conn->prepare("UPDATE request SET request_type=:request_type, request_desc=:request_desc, request_status='Permanent' WHERE request_id=:request_id");
