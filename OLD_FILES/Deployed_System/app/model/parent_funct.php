...
class ParentFunct
{
    ...
    public function updateProfile($prof_pic){       
        ...
        if($type=='image/jpg' || $type=='image/jpeg' || $type=='image/png' || $type=='image/gif' || $type=='application/pdf'  || $type=='audio/mpeg' || $type=='audio/mpeg3' || $type=='audio/x-mpeg' || $type=='audio/x-mpeg-3') { 
            if(!file_exists($pathWithFile)){
                if($size < 5000000){
                    ...
                    if($sql3->execute(array(
                        ...
                        $this->alert("Success!", "You have successfully changed your profile picture", "success", "parent-profile");
                    }else{
                        $this->alert("Error!", "You have successfully changed your profile picture", "error", "parent-profile");
                    }
                }else{
                    $this->alert("Error!", "Your file is too large! Please Upload 5MB Size", "error", "parent-profile");
                }
            }else{  
            ...
            $this->alert("Error!", "Upload JPG , JPEG , PNG & GIF File Formate.....CHECK FILE EXTENSION", "error", "parent-profile");
        }
    }
    public function parentDetails(){
        $queryParentDetails=$this->conn->prepare("SELECT *, CONCAT(guar_fname,' ',guar_midname,' ',guar_lname) as 'parentName' FROM guardian join accounts on acc_idx=acc_id WHERE acc_idx=:acc_idx");
        $queryParentDetails->execute(array(
            ':acc_idx' => $_SESSION['accid']
        ));
        if($queryParentDetails->rowCount() > 0){
            while($row=$queryParentDetails->fetch(PDO::FETCH_ASSOC)){
                $data[]=$row;
            }
            return $data;
        }
        return $queryParentDetails;
    }
        ...
        if ( password_verify($oldPassword, $user['password']) ) {
            if ($password === $repassword) {
                if (strlen($password) >= 8) {
                    ...
                    $this->alert("Success!", "You have successfully changed your password", "success", "parent-profile");
                    echo '<meta http-equiv="refresh" content="0"; url=admin-dashboard">';
                } else {
                    $this->alert('Error!',"Invalid password. Password length must be 8 characters and above",'error','parent-profile');
                } 
            } else {
                $this->alert('Error!',"Password doesn't match",'error','parent-profile');
            }
        }else {
            $this->alert('Error!',"Old Password doesn't match",'error','parent-profile');
        }
    }
    /*end profile*/
