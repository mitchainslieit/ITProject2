		...
		<div class="row">
			<div class="widget widgetParent">	
				...
				<div class="widgetContent ptaContent">
					<?php 
						$queryCount=$this->conn->prepare("SELECT * FROM treasurer join accounts on acc_trid=acc_id WHERE acc_type='Treasurer' and acc_status='Deactivated'");
						$queryCount->execute();
						$rowQueryCount=$queryCount->rowCount();
						
						$queryCount1=$this->conn->prepare("SELECT * FROM treasurer join accounts on acc_trid=acc_id WHERE acc_type='Treasurer'");
						$queryCount1->execute();
						$rowQueryCount1=$queryCount1->rowCount();
						if($rowQueryCount1 > 0){
							if($rowQueryCount > 0){
								echo '
								<div name="content">
