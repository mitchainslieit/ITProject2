						...
						<div name="content">
							...
							<div name="dialog" title="Add Class">
								<form action="superadmin-classes" method="POST" autocomplete="off">
									...
									<?php $rowCountSec=$obj->sectionTemp();	?> 	
...
if($acc_status === 'Deactivated'){
	echo '
		<tr class="overlayDeactivated">
		<td class="tleft">'.$fac_no.'</td>';
		echo $acc_status == "Deactivated" ? '<td class="tleft"><div class="temporaryContainer"><span class="temporary">'.$fullname.'</span><span class="temporaryDesc">This account has been deactivated!</span></div>' : ' <td class="tleft">'.$fullname.'</td>';
		echo '
			...
			<div name="content">
				...
				<div name="dialog" title="Update class data">
					<form action="superadmin-classes" method="POST" required autocomplete="off" class="validateChangesInForm">
						<h2>You are not allowed to edit this class, because the account of the adviser has been deactivated. You are required to re-assign an adviser in this section by using the add class button.</h2>
					</form>
				</div>  
			</div>
		</td>
