...
class SAdminFunct{
	...
	public function insertLogsFaculty($log_event, $log_desc){
		try {
			$sql2=$this->conn->prepare("SELECT fac_id from faculty WHERE sec_privilege='Yes' LIMIT 1");
			$sql2->execute();
			$rowSql2=$sql2->fetch(PDO::FETCH_ASSOC);
			$fac_id=$rowSql2['fac_id'];
	...
	private function insert_rank() {
		...
		$section = $getSection->fetchAll();
		foreach ($section as $sec) {
			$rank = $this->conn->prepare("SELECT ROUND(AVG(grade),2) as average, year_level, school_year, studd_id FROM grades join  student on studd_id = stud_id join subject on subj_ide = subj_id where secc_id = '1' GROUP by studd_id ORDER BY 1 DESC LIMIT 10");
		}
		/*$length_sec = count($getSection_length);
		for($i = 0; $i < $length_sec; $i++){
			$getTopTenStud = $this->conn->query("SELECT ROUND(AVG(grade),2) as average, year_level, school_year, studd_id FROM grades join  student on studd_id = stud_id join subject on subj_ide = subj_id where secc_id = '".$getSection_length[$i]['sec_id']."' GROUP by studd_id ORDER BY 1 DESC LIMIT 10");
			$rowCount = $getTopTenStud->fetchAll();
			if($getTopTenStud->rowCount() > 0){
				$length= count($rowCount);
				foreach($rowCount as $rc){
					$data[] = $rc;
				}	
			}
		}
