									...
									<form action="superadmin-system-settings" method="POST" class="validateChangesInForm">
										<div class="systemBox1">
											...
											if($queryCheckSyExist->rowCount() > 0){
												 foreach($obj->showSchoolYear() as $value){
													extract($value);
													echo $syStartCheck == '0000-00-00' ? '<input type="text" name="sy_start" readonly="readonly" data-validation="required" class="datepicker-schoolyearStart" data-validation="required" value="'.$school_start.'">' : '<input type="text" disabled name="sy_start" readonly="readonly" data-validation="required" class="datepicker-schoolyearStart" data-validation="required" value="'.$school_start.'">';
													echo'
														<span style="font-weight: 500 !important;">to</span>
														<input type="text" name="sy_end" readonly="readonly" data-validation="required" class="datepicker-schoolyearEnd" value="'.$school_end.'" data-validation="required">
													';
												}
											}else{
												echo '
													<input type="text" name="sy_start" readonly="readonly" class="datepicker-schoolyearStart" value="" data-validation="required"  data-validation-error-msg="Please fill out the field" 
     													data-validation-error-msg-container="#sy-error-dialog">
													<span style="font-weight: 500 !important;">to</span>
													<input type="text" name="sy_end" readonly="readonly" class="datepicker-schoolyearEnd" value="" data-validation="required"  data-validation-error-msg="Please fill out the field" data-validation-error-msg-container="#sy-error-dialog">
												';
											}
											?>
										</div>
										...
										<?php
											...
											if($currDate > $syStart){
												echo '
												<p class="tright">
													<button name="start-button" class="customButton">End <i class="fas fa-step-forward fnt"></i></button>
												</p>
												';
											}else{
												echo '
