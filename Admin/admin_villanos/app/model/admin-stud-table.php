...
class ListTable {
	...
	public function getStudents() {
		...
		if (isset($_POST['data'])) {
			if ($_POST['data'][0] === 'All') {
				$sql=$this->conn->prepare("SELECT stud_lrno, CONCAT(first_name, ' ', middle_name,' ', last_name) as 'Name',  year_level, sec_name, pay_amt, bal_amt, bal_status 
					...
					JOIN payment ON payment.pay_id = bp.pay_ida WHERE bal_status = :status GROUP BY 1");
				$sql->execute(array(
					':status' => $status
				));
			} else {
				$sql=$this->conn->prepare("SELECT stud_lrno, CONCAT(first_name, ' ', middle_name,' ', last_name) as 'Name',  year_level, sec_name, pay_amt, bal_amt, bal_status 
					...
					JOIN section on section.sec_id = student.secc_id 
					JOIN balpay bp ON bp.bal_ida = balance.stud_idb 
					JOIN payment ON payment.pay_id = bp.pay_ida WHERE year_level = :grade  AND bal_status = :status GROUP BY 1") or die ("failed!");
				$sql->execute(array(
					':grade' => $grade,
					':status' => $status
				));
